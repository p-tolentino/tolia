        ...
        <ScrollArea className="h-[calc(100vh-140px)] px-4 py-2">
          <nav className="flex flex-col gap-1">
            {navigationItems.filter((item) => {
              ...
              if (item.children) {
                return (
                  <Accordion type="single" collapsible key={item.title} defaultValue={hasActiveChild ? item.title : undefined}>
                    <AccordionItem value={item.title} className="border-none">
                      <AccordionTrigger className={cn(
                        "rounded-md px-3 py-3 text-sm font-medium transition-colors hover:bg-accent hover:no-underline",
                        (pathname === item.href || hasActiveChild) && "text-primary",
                      )}>
                        {item.title}
                      </AccordionTrigger>
                        ...
                        <div className="flex flex-col gap-1 pl-4">
                          {item.children.map((child) => (
                            ...
                            >
                              {child.title}
                              {child.badge && (
                                <span className="ml-2 text-[10px] text-primary">({child.badge})</span>
                              )}
                            </Link>
                          ))}
                        </div>
